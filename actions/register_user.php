...
try {
    ...
    // NOTA: Operações DDL (CREATE DATABASE) mais à frente causam "commit" implícito.
    // Por isso, a lógica de rollback manual no 'catch' é essencial.
    ...
    // Limpa resultados pendentes do multi_query
    ...
    if ($usuarioMasterId !== null) {
        ...
        try {
            // Deleta o tenant (usando o usuario_id, que é a FK)
            ...
            // Confirma a limpeza
            $conn->commit();
            error_log("INFO REGISTO: Rollback manual do usuario_id $usuarioMasterId foi bem-sucedido.");

        } catch (Exception $rollbackEx) {
            // Se a limpeza falhar, reverte e loga um erro crítico
            $conn->rollback();
            error_log("⛔ FALHA CRÍTICA NO ROLLBACK: Não foi possível limpar o usuário órfão $usuarioMasterId. Mensagem: " . $rollbackEx->getMessage());
        }
    }
    // --- FIM DA CORREÇÃO ---
