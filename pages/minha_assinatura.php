...
<head>
    ...
    <style>
        ...
        .card-custom { 
            ...
            box-shadow: 0 4px 15px rgba(0,0,0,0.3); transition: border-color 0.3s;
        }
        .card-custom:hover { border-color: #444; box-shadow: 0 4px 20px rgba(0, 191, 255, 0.1); }

        .card-title {
            color: #00bfff; font-size: 1.3rem; border-bottom: 1px solid #333;
        ...
        .neon-number {
            font-size: 3.5rem; font-weight: bold; color: #fff; line-height: 1; 
            text-shadow: 0 0 15px rgba(0, 191, 255, 0.6);
        }

        /* Buttons */
        .btn-custom {
            padding: 12px; border-radius: 6px; font-weight: bold; text-decoration: none;
            text-align: center; display: flex; align-items: center; justify-content: center;
            gap: 8px; border: none; cursor: pointer; color: #fff; transition: all 0.2s; font-size: 1rem; width: 100%;
        }
        ...
        .plan-card {
            ...
            justify-content: space-between; text-align: center; transition: transform 0.2s;
        }
            ...
            <div class="info-row">
                ...
                <span class="value">
                    ...
                    if ($tipo_cancelamento) {
                        ...
                        echo '<small style="color:#ccc; font-size: 0.85rem;">Encerra em: <strong style="color:#fff">' . $dias_ate_renovacao . ' dias</strong></small>';
                    } else {
            ...
            <?php if (!$is_trial && isset($dados_assinatura['data_renovacao'])): ?>
                <div class="info-row" style="margin-top: 15px;">
                    ...
                    <span class="value">
                        ...
                        <br>
                        <small style="font-size:0.8rem; color:#888; font-weight:normal;">(Faltam <?= $dias_ate_renovacao ?> dias)</small>
                    </span>
                </div>
            <?php endif; ?>
                ...
                <div class="plan-card" style="background: <?= $bg ?>; border: <?= $border ?>;">
                    ...
                    <?php else: ?>
                        <form id="formPlano_<?= $slug ?>" action="checkout_plano.php" method="POST">
                            ...
                            <button type="button" class="btn-custom" style="background: #00bfff;" onclick="confirmarTrocaPlano('<?= $slug ?>', '<?= $plano['nome'] ?>')">
...
<script>
    ...
    function confirmarAdicao() {
        Swal.fire({
            ...
            text: "Adicional de R$ 4,50/mês na fatura.",
            icon: 'info',
    ...
    function confirmarRemocao() {
        Swal.fire({
            ...
            if (result.isConfirmed) document.getElementById('formRemoveExtra').submit();
        });
    }
    ...
    function confirmarTrocaPlano(slug, nomePlano) {
        Swal.fire({
            ...
            if (result.isConfirmed) document.getElementById('formPlano_' + slug).submit();
        });
    }
</script>
