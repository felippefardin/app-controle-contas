...
<style>
  ...
  /* CSS BENEFICIOS (CUPOM/INDICAÇÃO) */
...
<script>
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (!senhaRegex.test(inputSenha.value)) {
      senhaError.textContent = "Senha fraca.";
      valid = false;
    ...
    if (document.getElementById('email').value !== email2.value) {
      emailError.textContent = "Emails não coincidem.";
      valid = false;
    } else {
    ...
    if (!valid) e.preventDefault();
        ...
        // Limpa storage ao enviar com sucesso (idealmente)
        localStorage.clear();
    }
  });
</script>
